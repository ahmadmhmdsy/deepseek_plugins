...
function linkRoot(name, target) {
  ...
  removeJunction(dest)
  // Scoped npm deps (e.g. @types/react) need their scope dir created first.
...
function resolvePkg(name, cwd) {
  try {
    ...
    // Packages whose "exports" does not expose ./package.json (e.g. vite-tsconfig-paths):
    // resolve the main entry, then walk up to the dir holding package.json.
    const entry = execFileSync('node',
      ['-e', 'console.log(require.resolve(' + JSON.stringify(name) + '))'],
      { cwd, encoding: 'utf8' }).trim()
