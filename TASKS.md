...
- **FE-M-A — View-only Monaco editor tab** `in_progress` —
  ...
  - [x] FE-M-A Task 3 — fs service + explorer tree + lazy Monaco workbench (host HALF done: FileEditorFs Remote service 'fileEditor' ns — @Remote list/read behind workspaceRegistry, policy engine src/remote/policy.ts with traversal/hidden/symlink-escape/8MB-cap/binary-sniff refusals, 12-test policy suite green RUN+DEV; composition check exit 0. REMAINING: client fs-remote contribution ($mount descriptors), explorer tree UI wiring, lazy Monaco chunk — now FE-M-A Task 3b) (see plan doc §Typert research: out-of-tree Remote namespaces ride the checkout's tsdown generator face plugin; host interactive candidate = DirectoryPicker-like service or generator-emitted typert.host/remote-client faces).
  - [x] FE-M-A Task 1 — tab path de-risk: conversation.view placeholder registered (id 'editor', order 15); bundle 3.03 kB; suite 106/106 (14 files) on RUN and DEV; tsc zero NEW errors (local slot shim; vendor-baseline noise documented in HANDOFF §8g); patch rows + profile delivery junction + composition check PASS — commit 364043f. **Live-GUI tab = user checkpoint (needs a harness boot of the web profile; never restart the running harness ourselves).**
